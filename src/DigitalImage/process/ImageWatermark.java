...
import DigitalImage.process.imageTrans.*;
import DigitalImage.process.Common;
...
public class ImageWatermark {
    ...
    public int[] chaosEmbed(int[] pix, int[] water, int iw, int ih) {
        for (int i = 0; i < iw * ih; i++) {
            ...
            //取(i,j)处灰度
//            11111111100010101000101010001010 & 0xff = 138
            gray = pix[i] & 0xFF;
...
//                    32 * 32 = 1024
                    if (block[i][j] > 1024)  //MNm=1024
                        dctWt[i][j] = dctIm[i][j];
                    else if (water[block[i][j]] == 255)
//                        d0 = -8 / 4 = -2
//                        d1 = 2 + 8 / 2 = 6
//                        step = 8
                        dctWt[i][j] = Math.round((dctIm[i][j] + d1[block[i][j]]) / step)
                                * step - d1[block[i][j]];
                    else
                        dctWt[i][j] = Math.round((dctIm[i][j] + d0[block[i][j]]) / step)
                                * step - d0[block[i][j]];
                }
            }
        }
...
//                    d1 = 2 + 8 / 2 = 6
//                    step = 8
                    sy0[i][j] = Math.round((dctWt[i][j] + d0[block[i][j]]) / step)
                            * step - d0[block[i][j]];
                    sy1[i][j] = Math.round((dctWt[i][j] + d1[block[i][j]]) / step)
                            * step - d1[block[i][j]];

                    int k = block[i][j];
                    t1[k] = t1[k] + (dctWt[i][j] - sy1[i][j]) * (dctWt[i][j] - sy1[i][j]);
                    t0[k] = t0[k] + (dctWt[i][j] - sy0[i][j]) * (dctWt[i][j] - sy0[i][j]);
                }
            }
        }
        ...
        for (int k = 0; k < 1024; k++) {
            if (t0[k] - t1[k] > 0) {
                water[k] = (255 << 24) | (255 << 16) | (255 << 8) | 255;
            } else {
                water[k] = (255 << 24) | (0 << 16) | (0 << 8) | 0;
            }
        }
        return water;
    }
